...
function getSiteInfo({ collection, block, siteBlock, NOTION_CONFIG }) {
  ...
  const defaultIcon = siteConfig('AVATAR', '/avatar.svg', NOTION_CONFIG)
  ...
  const description = collection?.description
    ...
    : defaultDescription
  const pageCover =
    (siteBlock?.format?.page_cover
      ? mapImgUrl(siteBlock.format.page_cover, siteBlock, 'block')
      : null) ||
    (collection?.cover
